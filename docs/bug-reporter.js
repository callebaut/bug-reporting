...
(function () {
  ...
  var BugReporter = (function () {
    ...
    // Collect a storage object. Only the listed keys are included (allowlist);
    // if no keys are configured, nothing is collected by default.
    function collectStore(store, keys) {
      ...
      try {
        ...
        // no keys configured → collect nothing
    ...
    function collectCookies(keys) {
      ...
      // no names configured → collect nothing
      return sel;
    }
    ...
    function buildMarkdown(c, imageNote) {
      ...
      if (c.cookies && Object.keys(c.cookies).length) {
        L.push('## Cookies');
        L.push(fence(JSON.stringify(c.cookies, null, 2), 'json'));
        L.push('');
      }
      if (c.localStorage && Object.keys(c.localStorage).length) {
        L.push('## localStorage');
        L.push(fence(JSON.stringify(c.localStorage, null, 2), 'json'));
        L.push('');
      }
      if (c.sessionStorage && Object.keys(c.sessionStorage).length) {
    ...
    function buildHtml(c, imgDataUrl) {
      ...
      if (c.headers && Object.keys(c.headers).length) pre('Response headers', c.headers);
      if (c.cookies && Object.keys(c.cookies).length) pre('Cookies', c.cookies);
      if (c.localStorage && Object.keys(c.localStorage).length) pre('localStorage', c.localStorage);
      if (c.sessionStorage && Object.keys(c.sessionStorage).length) pre('sessionStorage', c.sessionStorage);
